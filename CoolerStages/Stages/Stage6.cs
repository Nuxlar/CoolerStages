            ...
            if (terrainMat && detailMat && detailMat2 && detailMat3)
            {
                ...
                foreach (MeshRenderer renderer in meshList)
                {
                    ...
                    if (meshBase != null)
                    {
                        ...
                        if ((meshBase.name.Contains("Bridge") || meshBase.name.Contains("MoonPillar") || meshBase.name.Contains("Roof") || meshBase.name.Contains("Arena") || meshBase.name.Contains("MoonPlatform") || (meshBase.name.Contains("Quarry") && !meshBase.name.Contains("Fog")) || meshBase.name.Contains("Connector") || meshBase.name.Contains("Octagon") || meshBase.name.Contains("Temple") || meshBase.name.Contains("Column") || meshBase.name.Contains("CoolingBowlLarge") || (meshBase.name.Contains("Wall") && !meshBase.name.Contains("CylinderWall")) || meshBase.name.Contains("Floor") || meshBase.name.Contains("Barrier") || meshBase.name.Contains("WaterTrough") || meshBase.name.Contains("Busted") || meshBase.name.Contains("disc") || meshBase.name == "Base") && renderer.sharedMaterial)
